...
public class CorrelationQuality extends PixelBenchmark implements Benchmark
{
    public double calculate(final BufferedImage original, final BufferedImage stego) {
        ...
        for (int i = 0; i < original.getHeight(); ++i) {
            for (int j = 0; j < original.getWidth(); ++j) {
                ...
                originaldiff += Math.abs(this.getRed(origpix)) + Math.abs(this.getGreen(origpix)) + Math.abs(this.getBlue(origpix));
